...
function grabBlightCard(blightDeck)
    ...
    local card = blightDeck.takeObject({
        ...
        callback_function = function(obj) if not useEventDeck and (not obj.getVar("healthy") and (obj.getVar("immediate") or obj.getVar("blight") == 2)) then obj.setRotationSmooth(Vector(0,180,0)) grabBlightCard(blightDeck) else blightedIslandCard = obj blightedIslandCard.setPositionSmooth(blightCardPos) blightedIslandCard.setLock(true) addBlightedIslandButton() end end,
    })
end
...
function BlightedIslandFlip()
    ...
    if not blightedIslandCard.is_face_down then
        -- still healthy
        local cardPos = blightedIslandCard.getPosition()
        blightedIslandCard.setPositionSmooth(Vector(-61.86, 1.5, 26.62))
        blightedIslandCard.setLock(false)
        blightDeck = getObjectFromGUID("b38ea8").getObjects()[1]
        if blightDeck == nil then
            hideBlightButton()
            return
        elseif blightDeck.tag == "Deck" then
            blightDeck.shuffle()
            blightedIslandCard = blightDeck.takeObject({
                position = cardPos,
                rotation = Vector(0,180,0),
                -- need to wait for card to exist outside of deck
                callback_function = function() startLuaCoroutine(Global, "BlightedIslandFlipPart2") end,
            })
            blightedIslandCard.setLock(true)
            return
        elseif blightDeck.tag == "Card" then
            blightedIslandCard = blightDeck
            blightedIslandCard.setPositionSmooth(cardPos)
            blightedIslandCard.setLock(true)
        else
            hideBlightButton()
            return
